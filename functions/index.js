...
// ══════════════════════════════════════════════
//
// REQUISITOS (secrets de Firebase Functions):
...
// Validate Twilio config on startup
if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN) {
  logger.warn('⚠️ TWILIO_ACCOUNT_SID or TWILIO_AUTH_TOKEN not set. Configure Firebase Functions secrets before deploying.');
}
...
// ══════════════════════════════════════════════
function twimlSay(text, voice = 'Polly.Lucia') {
  return `<Say voice="${voice}" language="es-US">${escapeXml(text)}</Say>`;
}

function twimlGather(text, action, method = 'POST', timeout = 6, numDigits = 1, speechTimeout = 'auto') {
  return `<Gather input="speech dtmf" action="${action}" method="${method}" timeout="${timeout}" numDigits="${numDigits}" speechTimeout="${speechTimeout}">
    ${twimlSay(text)}
  </Gather>`;
}
...
async function maryResponde(transcripcion, historial, callSid, salonConfig) {
  try {
    ...
    return 'Disculpa, tuve un pequeño problemita de conexión. ¿Podrías repetir lo que necesitas, por favor?';
  }
}
...
async function guardarLlamada(callSid, data) {
  try {
    await db.collection('llamadas').doc(callSid).set({ ...data, ts: Date.now(), fecha: new Date().toISOString() });
  } catch (e) { logger.error('Error guardando llamada:', e); }
}

async function guardarCita(data) {
  try {
    const ref = await db.collection('citas_bot').add({ ...data, ts: Date.now(), estado: 'pendiente', fecha: new Date().toISOString() });
    return ref.id;
...
async function guardarMensaje(callSid, role, text) {
  try {
    ...
    if (mensajes.length > 10) mensajes.splice(0, mensajes.length - 10);
    await ref.set({ mensajes, callSid, ultimaActualizacion: Date.now() });
...
exports.incomingCall = onRequest({ cors: true }, async (req, res) => {
  try {
    ...
    await guardarLlamada(callSid, { callSid, callerNumber, estado: 'activa' });

    const config = await getSalonConfig();

    const saludo = `Hola, bienvenida a ${config.nombre}. Soy ${config.asistente}, tu asistente virtual. ¿En qué te puedo ayudar hoy? Puedo contarte sobre nuestros servicios, precios, horarios, o ayudarte a agendar una cita.`;

    const twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  ${twimlSay(saludo)}
  ${twimlPause(1)}
  ${twimlGather('Dime, ¿qué necesitas?', `/conversar?callSid=${callSid}`, 'POST', 6, 1, 'auto')}
...
exports.conversar = onRequest({ cors: true }, async (req, res) => {
  try {
    ...
    logger.info(`🗣️ Conversación ${callSid}: speech="${speechResult}" digits="${digits}"`);
...
<Response>
  ...
  <Hangup/>
    ...
    if (intencion === 'despedida') {
      await guardarLlamada(callSid, { estado: 'finalizada', motivo: 'despedida' });
      const twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  ${twimlSay('¡Ha sido un gusto atenderte! Recuerda que estamos de lunes a domingo de 8 AM a 8 PM. ¡Que tengas un día hermoso!')}
  <Hangup/>
</Response>`;
      res.type('text/xml').send(twiml);
      return;
    ...
    if (intencion === 'dueno') {
      ...
      const twiml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  ${twimlSay('Claro, con mucho gusto voy a dejarle un mensaje a la dueña del salón para que te contacte lo antes posible. ¿Me dejas tu nombre y número para que te pueda devolver la llamada?')}
  ${twimlGather('', `/conversar?callSid=${callSid}`, 'POST', 6, 1, 'auto')}
  ${twimlPause(1)}
  ...
  <Hangup/>
    ...
    const quizoAgendar = transcripcion.toLowerCase().match(/agend|cita|reserv|turno/) || 
                         respuestaMary.toLowerCase().match(/¿qué día|¿qué hora|¿para cuándo|fecha.*prefer/);

    if (intencion === 'agendar' || quizoAgendar) {
      await guardarCita({
        callSid,
        transcripcion: transcripcion,
        respuestaMary: respuestaMary,
...
<Response>
  ...
  ${twimlPause(1)}
  ${twimlGather('', `/conversar?callSid=${callSid}`, 'POST', 6, 1, 'auto')}
  ${twimlSay(`Muchas gracias por llamar a ${config.nombre}. Esperamos verte pronto. ¡Que tengas un lindo día!`)}
  <Hangup/>
</Response>`;
...
exports.callStatus = onRequest({ cors: true }, async (req, res) => {
  ...
  if (['completed', 'no-answer', 'failed', 'busy'].includes(callStatus)) {
    await guardarLlamada(callSid, { estado: callStatus, duracion: parseInt(duration), fechaFin: new Date().toISOString() });
  }
...
exports.getStats = onCall(async () => {
  try {
    ...
    const citasPendientes = citas.filter(c => c.estado === 'pendiente' || c.estado === 'pendiente_confirmacion').length;
    const duraciones = llamadas.filter(l => l.duracion).map(l => l.duracion);
    const duracionPromedio = duraciones.length ? Math.round(duraciones.reduce((a, b) => a + b, 0) / duraciones.length) : 0;

    return { success: true, data: { llamadasHoy, citasHoy, citasPendientes, llamadasTotal: llamadas.length, duracionPromedio, citasTotal: citas.length } };
  } catch (error) {
